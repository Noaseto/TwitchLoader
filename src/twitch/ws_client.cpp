...
void WsClient::start(const std::string& client_id, const std::string& oauth) {
    if (!m_running) {
        m_running = true;
        svc_log->info(mod_ctx, LOG_START_WEBSOCKET.data());
        m_thread = std::thread([this, client_id, oauth] { run(client_id, oauth); });
    }
}

void WsClient::push_message(const TwitchEventType type, const std::string& message) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
void WsClient::connect_and_subscribe(const std::string& client_id, const std::string& oauth,
    net::io_context& ioc, ssl::context& ctx, websocket::stream<ssl::stream<tcp::socket>>& ws) {
    tcp::resolver resolver{ioc};
    std::string user_id = get_user_id(client_id, oauth, ioc, ctx, resolver);

    std::string host = TWITCH_WEBSOCKET_URL.data();
    std::string port = HTTPS_PORT.data();
    ...
    for (const auto& topic : topics) {
        ...
        body[JSON_TRANSPORT.data()] =
            json{{JSON_METHOD.data(), JSON_WEBSOCKET.data()}, {JSON_SESSION_ID.data(), session_id}};
...
void WsClient::running_loop(websocket::stream<ssl::stream<tcp::socket>>& ws) {
    while (m_running) {
        beast::flat_buffer buffer;
        beast::error_code ec;
        ws.read(buffer, ec);
        ...
        if (message_type == JSON_MESSAGE_TYPE_SESSION_RECONNECT.data()) {
            push_message(TwitchEventType::Unknown, data);
            break;
        }
        ...
        push_message(type, data);
    }
}

void WsClient::shutdown(websocket::stream<ssl::stream<tcp::socket>>& ws) {
...
void WsClient::run(const std::string& client_id, const std::string& oauth) {
    net::io_context ioc;
    ssl::context ctx{ssl::context::tlsv12_client};
    ctx.set_default_verify_paths();
    websocket::stream<ssl::stream<tcp::socket>> ws{ioc, ctx};
    try {
        connect_and_subscribe(client_id, oauth, ioc, ctx, ws);
        running_loop(ws);
    } catch (std::exception const& exception) {
        if (m_running) {
            push_message(TwitchEventType::TwitchEventError,
