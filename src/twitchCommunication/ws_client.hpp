...
private:
    ...
    void run(const std::string& host, const std::string& port, const std::string& clientId,
        const std::string& oauth, const std::string& username, const std::string& userId) {
        ...
        try {
            ...
            for (const auto& topic : topics) {
                ...
                body[JSON_TRANSPORT.data()] = json{{JSON_METHOD.data(), JSON_WEBSOCKET.data()},
                ...
                request.set(
                    beast::http::field::authorization, TWITCH_API_AUTHORIZATION.data() + oauth);
            ...
            while (m_running) {
                ...
                svc_log->info(
                    mod_ctx, (subscription_type + LOG_MESSAGE_TYPE_RECEIVED.data()).c_str());
